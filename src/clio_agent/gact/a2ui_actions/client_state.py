...
2. **Client error reports** (the official ``{version, error: {...}}``
   ...
   cannot be resolved. ``VALIDATION_FAILED`` gets exactly ONE repair
   delivery per surface revision; a second report for the same revision
   marks the surface ``state="failed"`` (``gact/a2ui_actions/record.py::
   fold_action_records`` applies that projection rule); a generic error code
   is persisted for the record but never delivered.
"""
