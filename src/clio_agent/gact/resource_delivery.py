...
DELIVERY_REASONS: dict[str, str] = {
    ...
    "native_image_modality_unknown": (
        "no evidence establishes whether this model accepts image input; unknown is not "
        "refused, so the image is delivered natively and the upstream endpoint decides"
    ),
...
def plan_resource_delivery(
    ...
    *,
    ...
    if media_type.startswith(NATIVE_MEDIA_PREFIXES) and not evidence.known:
        # Unknown image input is not a refusal (modality_evidence.image_input_capability):
        # the image rides natively under a typed reason naming the unknown.
        representation: DeliveryRepresentation = "native"
        reason_code = "native_image_modality_unknown"
        reason = DELIVERY_REASONS[reason_code]
    else:
        representation, reason_code, reason = _representation_for(
            media_type, set(evidence.modalities or {"text"})
        )
    return ResourceDeliveryRecord(
        ...
        evidence_source=evidence.evidence,
