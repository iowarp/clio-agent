...
def _runtime_dynamic_agent_children_context(
    ...
    *,
    ...
    lines.append(
        ...
        "with `wait_agent_tasks([task_id, ...])` as one committed wait when your next step "
        "depends on all of them; it returns once the requested children are terminal, so do "
        "not repeatedly poll or narrate that they are still running. Pass `timeout_s=...` "
        "only when you intentionally need a finite progress checkpoint. "
