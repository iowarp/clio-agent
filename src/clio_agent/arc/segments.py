    ...
    def _segs(self, session_id: str, scope: str) -> list[Segment]:
        ...
        if key not in self._loaded:
            ...
            logger.debug(
                ...
                session_id, scope, len(segs), self._next_lt,
            )
        ...
        *,
        ...
        with self._lock:
            ...
            logger.debug(
                ...
                scope, kind, step, seg.logical_time, seg.order, token_count, seg.id,
            )
            self._finish_write(
                session_id, scope, "append", written=[seg], step=step,
        ...
        *,
        ...
        with self._lock:
            ...
            logger.debug(
                ...
                scope, position, kind, seg.logical_time, seg.order, seg.id,
            )
            self._finish_write(
                session_id, scope, "insert", written=[seg], step=step, position=position,
                logical_time=seg.logical_time,
            )
            return seg
        ...
        with self._lock:
            ...
            if tombstoned:
                logger.debug(
                    ...
                    scope, len(tombstoned), len(ids), op_lt, tombstoned,
                )
                self._finish_write(
        ...
        with self._lock:
            ...
            if tombstoned:
                logger.info(
                    ...
                    scope, len(tombstoned), len(ids), summary_lt, token_count, live_remaining,
                )
            else:
                logger.warning(
                    "segments: summarize scope=%s matched no live ids=%s "
                    "(summary appended only)",
                    scope, ids,
                )
            self._finish_write(
                session_id,
    ...
    def render(self, session_id: str, scope: str, *, as_of: int | None = None) -> list[Segment]:
        ...
        with self._lock:
            ...
            visible = [
                s
                for s in segs
                if s.logical_time <= as_of
                and (s.tombstoned_at == 0 or s.tombstoned_at > as_of)
            ]
    ...
    def render_text(
        ...
        *,
        ...
        return separator.join(
            segment_text(s) for s in self.render(session_id, scope, as_of=as_of)
        )
