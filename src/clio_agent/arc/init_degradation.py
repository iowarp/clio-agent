...
When ARC is configured for the clio-core backend (the default) but clio-core
...
* a **process-local record** other in-process code can read -- the doctor
  ...
  (:func:`clio_agent.runtime.clio_core_health.probe_clio_core_init_degradation`), mirroring
...
class ArcInitDegradation:
    """A recorded init-time degrade from the clio-core backend to LocalFS (#897)."""
...
# per process (ClioCoreStore._initialized guard), so a single latest-record slot is
...
def _clio_core_selected_explicitly(backend: str | None) -> bool:
    """Whether the clio-core backend was chosen explicitly (arg or env) vs the default.
...
def record_arc_init_degradation(
    *,
    ...
    Args:
        ...
        error: The exception raised during clio-core init.
    ...
    record = ArcInitDegradation(
        reason=reason,
        choice="cte",
        was_explicit=_clio_core_selected_explicitly(backend),
    ...
    logger.warning(
        "ARC degraded to LocalFSStore at init: the clio-core backend is "
