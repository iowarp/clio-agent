"""External Agent Compiler for ClioAgent

Compiles external agents (LangChain, CrewAI, AutoGen) into ClioAgent-compatible format.
...
Example:
    >>> from clio_agent.registry.external_compiler import ExternalAgentCompiler, ExternalAgentDefinition
    >>>
...
from clio_agent.registry.a2a_adapter import A2AProtocolHandler, A2ARequest, A2AResponse
...
class ExternalAgentWrapper:
    """Wraps external agent for ClioAgent compatibility.
...
class ExternalAgentCompiler:
    """Compiles external agents into ClioAgent format.

    Converts external agent definitions (LangChain, CrewAI, AutoGen)
    into ClioAgent-compatible wrappers that can be registered in AgentRegistry.
    ...
    Example:
        ...
        >>>
        >>> # Register in ClioAgent registry
        >>> registry.register_agent("langchain_sql", wrapper, capabilities)
        >>>
        >>> # Use like any ClioAgent agent
    ...
    def compile_agent(self, definition: ExternalAgentDefinition) -> ExternalAgentWrapper:
        """Compile external agent definition into ClioAgent-compatible wrapper.
