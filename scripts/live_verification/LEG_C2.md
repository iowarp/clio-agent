...
| 4 | cache | No | **pass** (C1-S5, #1285, landed) | `tools/mcp_runtime.py::make_mcp_client` opts every execution-path client into SEP-2549 response caching by default (`response_cache_enabled()`, config `tools.mcp.response_cache_enabled`); `mcp_exerciser.py::build_exerciser_server` accepts `cache_ttl`/`cache_scope` (fastmcp applies the hint server-wide, there is no per-tool knob). The avenue asserts a hinted server's second `tools/list` is served from cache (a recording store proves exactly one `set`, not two). |
