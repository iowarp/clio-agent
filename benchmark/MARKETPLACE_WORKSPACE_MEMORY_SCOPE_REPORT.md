...
Generated: 2026-06-03 07:27:40 CDT
Evidence JSONL: `/tmp/clio-memory-scope-benchmark/MARKETPLACE_WORKSPACE_MEMORY_SCOPE_EVIDENCE.jsonl`
...
- Max elapsed case: `workspace_memory_scope_policy` (0.0s)
- Max expert depth: `workspace_memory_scope_policy` (0)
- Max branch fanout: `workspace_memory_scope_policy` (0)
...
- Declared semantic proofs: workspace_memory_scope
- Observed semantic proofs: workspace_memory_scope
...
| observed semantic proof coverage by case | command_mcp_skill_scope=[], enabled_mcp_execution=[], failure_recovery=[], marketplace_pack=[], nested_tier3=[], no_shortcuts=[], packaged_hook_invocation=[], root_delegation=[], sync_parent_return=[], workspace_memory_scope=['workspace_memory_scope_policy'] | reported | pass |
...
- root_delegation: root Agent delegates through declared experts
- sync_parent_return: sync child delegation returns control to the parent
- workspace_memory_scope_policy: route_metrics={'expert_depth': 0, 'branch_count': 0, 'child_session_branch_count': 0, 'sync_handoff_count': 0, 'unique_experts': 0, 'unique_tools': 0, 'tool_call_count': 0, 'artifact_count': 0}

## Semantic Proof Declarations

| Case | Declared | Observed |
| --- | --- | --- |
| workspace_memory_scope_policy | workspace_memory_scope | workspace_memory_scope |

## All Cases

| Case | Category | Blueprint | Mode | Source | Outcome | Agent | Handoffs | Tools | Children | Elapsed |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| workspace_memory_scope_policy | memory-scope | - | auto | - | pass | - | - | - | 0 | 0.0s |

## Best 10 Demo Prompts

### 1. Workspace memory scope policy

Case: `workspace_memory_scope_policy`
...
Actions: workspace_memory_scope_probe=ok
Child sessions: none
Artifacts: none
...
```text
Using the current workspace, prove that cross-session memory search requires explicit user intent, same-workspace memory can be read with that intent, and another workspace's session memory does not leak.
```

What to see: CLIO records denied cross-session memory search without intent, allows same-workspace memory search with user intent and provenance, and denies a read of another workspace's session summary.

Why this is interesting: Turns workspace memory compartmentalization from API-only tests into committed benchmark evidence for the 1.0 semantic regression lane.
