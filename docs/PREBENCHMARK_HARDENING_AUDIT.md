...
- Existing open benchmark/deferred issues:
  ...
  - Live-provider multimodal smoke evidence during the manual benchmark
...
| Context files and attachments | Workspace-relative context files, content preview endpoint, attachment upload, confinement, large/invalid file handling, message injection, attachment media-type provenance, image `Part` transcript preservation/provider gating, and native image forwarding into the CLIO/DSPy orchestrator path are tested. | Context file/content/attachment tests; file mention docs; image-part GACT tests; CLI provider fail-fast tests; core planner image-forwarding test. | Live OpenAI/Anthropic image-provider proof is still a manual benchmark item; local CI verifies conversion/forwarding but does not spend external provider calls. | Text/data ready; image preservation, gating, and native forwarding implemented. |
| Provider configuration and swaps | Provider catalog/configuration, ALCF token status, LM Studio/Ollama/OpenAI-compatible paths, async provider swap for blocking providers, and provider provenance are tested. | Provider tests; benchmark runner supports provider-swap case. | Provider-swap stress remains future benchmark coverage; ALCF readiness depends on environment token status during benchmark. | Implemented/tested; stress proof pending final benchmark. |
| Deterministic shortcut removal | Keyword user-agent routing is opt-in; benchmark lanes reject shortcut route sources and require root-owned sync delegation evidence; legacy ARC keyword coordination is disabled by default and explicitly classified as non-runtime compatibility infrastructure. | `docs/DETERMINISTIC_SHORTCUT_AUDIT.md`; `tests/test_gact/test_post_messages.py`; `tests/test_scripts/test_run_demo_benchmark.py`; `tests/test_arc/test_coordinator.py`; benchmark reports. | NDP/SAC recovery guards remain domain-specific safety checks and should be reviewed against the next real-provider trace to confirm the planner chose recovery actions. | Classified and guarded; final benchmark review pending. |
| Marketplace benchmark evidence | Current marketplace evidence proves multiple marketplace Blueprints, root-owned sync delegation, non-seismic complex hierarchy paths, one complex seismic chain, focused MCP/hook/memory proofs, and a verified seismic PNG artifact. The `marketplace_agents` lane has a strict gate for complex hierarchy coverage: at least three passing marketplace cases must show depth >= 3, branch count >= 2, sync handoff count >= 2, and complete parent-return provenance. | `benchmark/CURRENT_STATUS.md`; current marketplace benchmark reports; `tests/test_scripts/test_run_demo_benchmark.py`. | The final public-demo dry run should regenerate the frozen pack set from a fresh machine/session before broad marketplace hierarchy claims. | Harness guarded; final manual benchmark proof pending. |

## Critical Findings

1. **Packaged hooks now have focused marketplace evidence, but not broad
   real-provider task evidence.** Runtime hooks, Agent Blueprint packaged hook
   enablement, and packaged message-hook semantic trace provenance are
   implemented and the `hook-smoke` focused benchmark proves explicit
   trust/enable plus `pre_message` invocation. CLIO should still avoid claiming
   broad hook readiness for scientific workflows until a real-provider
   marketplace task uses hooks as part of a larger domain run.
