...
| Marketplace pack completeness | Packs can package experts, prompt bodies/ids, skills refs, command recipe files, command allowlists, built-in tool refs, MCP descriptors, packaged hooks, prompt profiles, provider/model defaults. | Marketplace files plus tests for parser/runtime surfaces. `remote-sensing-ogc-review` contains one MCP descriptor; packaged hooks and commands have backend/API proof. | Current marketplace packs are mostly shallow except seismic; command and provider/model/profile/hook semantics still need real-provider marketplace evidence. | Open benchmark gaps #510, #511, #516. |
