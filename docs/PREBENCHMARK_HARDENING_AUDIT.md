...
- Existing open benchmark/deferred issues:
  ...
  - #528 native model-visible image request support
...
| Context files and attachments | Workspace-relative context files, content preview endpoint, attachment upload, confinement, large/invalid file handling, message injection, attachment media-type provenance, and image `Part` transcript preservation/provider gating are tested. | Context file/content/attachment tests; file mention docs; image-part GACT tests; CLI provider fail-fast tests. | Native model-visible image dispatch through the CLIO/DSPy hierarchy is still pending #528; image turns are preserved and text-only providers fail fast instead of silently dropping images. | Text/data ready; image preservation/gating implemented; native vision pending #528. |
