...
| **S3** child-turn substrate: `spawn_child_turn` (#951) | #983 | `gact/turn_spawn.py`: `spawn_child_turn(app, TaskSpec) → AgentTask` mints a child session + stages a **real turn** via the S1 runner; `spawn_child_turn_threadsafe` for the parent's forward thread. Dedicated child-turn `ThreadPoolExecutor` (a waiting parent can't starve children). Guards: declared-only, depth>3 refused, FIFO queue admission at the cap. `TaskSpec` serializable — the **#671 seam** minted. | — | 2 majors: cancel-cascade freed slots but never admitted queued tasks; executor never shut down — both fixed | Real claude_code child turn completed with result `message_ref` + real answer + full child transcript + parent-visible `queued→started→completed`. 7 tests |
