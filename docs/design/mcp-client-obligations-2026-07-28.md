...
| D2 | listChanged handling | invalidate + re-fetch | Optional | handler hooks | DONE (#1285 C1-S5 item 2, wired into PRODUCTION in the review round): `tools/mcp_runtime.py::make_mcp_client` folds `tools/mcp_listen.py::combined_message_handler(server_id)` into EVERY execution-path client it builds whenever `server_id` is known (composed with any caller-supplied message hook, never overwriting it) — closing the gap where `watch_list_changed`/`list_changed_message_handler` had zero production callers before this (`test_make_mcp_client_wires_list_changed_invalidation_when_server_id_known`); live-verified (leg_c2 avenue "list-changed") |
...
| E11 | Cacheable results (SEP-2549) | ttlMs + cacheScope on discover/list/read; key by method+params; MUST NOT cache MRTR-retry results; "private" scope per authorization context; invalidate on notifications; no TTL polling without jitter | Required fields | v4 respects hints; pluggable store | CAVEAT: opt-in, default OFF (`response_cache_enabled()`, config `tools.mcp.response_cache_enabled`, env `CLIO_MCP_RESPONSE_CACHE_ENABLED`) — an operator must explicitly enable it before any of the below applies. DONE (#1285 C1-S5 item 3): `tools/mcp_runtime.py::make_mcp_client` then opts execution-path clients into the SDK's own `mcp.client.caching` mechanism. The MUST-NOTs are structural BY CONSTRUCTION, NOT independently tested — only list/read methods are ever cacheable (`tools/call` and any MRTR retry are excluded because clio never passes them through the cache-eligible call path; fastmcp's own eviction wrapper invalidates on listChanged/resourceUpdated) — gate before enabling in a security-sensitive deployment rather than treating "structural" as "verified". Live-verified (leg_c2 avenue "cache"; `test_mcp_response_cache.py`) — separate from and in addition to `tools/mcp_listen.py`'s push-invalidation of the boot listing cache (now wired into every `server_id`-known production client, #1285 review round MUST 1) |
...
| F7 | Sampling (+tools) | DEPRECATED; we never implemented — MUST NOT add | Deprecated | full handler exists | owner-ruled: the `createMessage` PRIMITIVE stays un-added (absent stays absent; `sampling_callback`/`CreateMessageRequest`/`SamplingFnT` remain zero-hit, `test_mcp_era_gated_removals.py`). C1-S7 (#1309) delivers the two legitimate residual SEMANTICS sampling would have served — a multi-provider, per-session model identity and conversation-context-aware answers — through **agent-driven elicitation** instead: an MCP server's own `input_required`/MRTR question, `_meta`-marked for the agent, routed under policy to the session's agent and answered as a typed, schema-validated reply to the server's OWN declared question (never free-form completions, never model access, never prompt control — the requestedSchema validation is the semantic firewall). This is question routing, not a sampling primitive; the ratchet stands unmodified and un-widened |
...
| H8 | Credential↔issuer binding (SEP-2352) | key stored credentials by AS issuer; never reuse across ASes; priority pre-reg→CIMD→DCR→prompt | Required | storage backends exist; issuer-keying confirmed (`credentials_match_issuer`, `test_mcp_verification_probe_shortlist.py::test_h8_...`) | CAVEAT FIRST: `tools/mcp_oauth_storage.py::DurableFileTokenStorage` (OURS, #1285 item 5) is keyed by `server_url` — the identity available at construction time — NOT by AS issuer; H8's ideal per-AS-issuer keying needs metadata discovery that happens later than storage construction, so two servers sharing one AS get two independent credential entries today (safe, just not the SEP-2352 ideal). The SDK's OWN in-flight `credentials_match_issuer` check is verified correct — that is the part of this row that is "verified", not CLIO's storage keying |
