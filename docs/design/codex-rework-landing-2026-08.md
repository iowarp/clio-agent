...
- **W1 — structural gates:** `protocol_v3.py` is now a compatibility facade over `gact/protocol/v3/{workspace,session,message,event,capabilities}.py`; oversized owners were decomposed into bounded modules rather than trimmed around the ratchet. Every modified file-size baseline moved downward. The route inventory moved from 193 to 205 because twelve route/method pairs were moved into named route owners and made visible to the inventory; this is an exact intentional inventory update, not a maximum-size or fallback-baseline waiver. Environment references were regenerated from source. Ruff, mypy over `src/` (450 files), file-size, class-in-function, delegation-vocabulary, settle-vocabulary, tool-instrumentation, and silent-fallback guards passed locally; the silent-fallback baseline remains 1.
- **W2 — proven correctness:** corrupt A2UI state is quarantined with a typed load reason; session defaults reject partial model refs and reconcile provider changes; A2UI batches validate before mutation and deletion is terminal; tool failure derives from the unbounded structured result while the model lane remains bounded; eviction, removed-transport, cancellation/reconciliation, skill inventory, and telemetry contracts have dedicated assertions. No acceptance was converted to a skip.
...
- **Focused/stress evidence:** all structural guards above passed; the concurrency flake-hunt passed with **270 passed**. The post-message route module passed **43/43** under Python 3.12 after lifecycle correction. The tool-metadata module passed **3/3** under Python 3.13, and the formerly intermittent expert-handoff assertion passed **10/10** consecutive repetitions. Affected-file Ruff, file-size, and diff checks passed. No broad local suite was substituted for GitHub CI and no ratchet baseline was raised.
