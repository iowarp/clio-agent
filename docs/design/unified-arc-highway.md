...
>   change to what gets *written* (step-1 fold, dedup-as-op, redaction-at-ingest,
>   part atoms, streaming atom) escapes it and needs a **live dual-run A/B** harness
...
- The log lands on `ARCStore` (`arc/storage.py`): **CTE** when `CLIO_ARC_STORE=cte`, **LocalFS** msgpack chunks otherwise (today's default). CTE offload is proven sha256-identical (`tests/test_arc/test_cte_offload_spill.py`). Canonical-store residency is CTE's tiering job (owner ruling); LocalFS keeps the O(chunk) append as the durable floor.
...
- **Retention / erasure** (the append-only tension the review flagged):
  ...
  - **LocalFS growth** is bounded by the same `DELETE`/retention TTL that bounds today's `messages/*.json`; the log does not grow *faster* than the copies it replaces (it replaces them). CTE installs offload past the hot cap.
...
3. **Re-express serving-layer dedup as op/fold** — #736 `_dedup_cross_agent_text` becomes `summarize`/tombstone ops (#737 scope 3). *Write-path.* Gate: **live dual-run** SSE diff (1.2) — NOT the captured corpus (a captured log predates the dedup ops; §gate-asymmetry).
...
- **(b) session-scoped, so no double-apply.** The flag is evaluated at **session creation** and pinned for the session's life. A session is EITHER old-regime OR new-regime end-to-end — never both. So rolling back step 3 (dedup-as-op) cannot double-dedup a session: sessions written under step 3 keep op-dedup (their fold applies it once); sessions created after rollback use assembly dedup (once). No session ever gets both passes.
...
1. **Physical residence of the canonical log by default — CTE or LocalFS?**
   *Rec:* LocalFS msgpack chunk-family stays the default durable floor (O(chunk) append); CTE opt-in (`CLIO_ARC_STORE=cte`) owns residency/tiering when on. The collapse must hold with ARC=local (the profile ran local). Do not make CTE a hard dependency.
