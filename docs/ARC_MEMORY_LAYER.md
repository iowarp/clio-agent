...
The canonical, source-derived list lives in [ENVIRONMENT.md](ENVIRONMENT.md)
(generated by `scripts/gen_env_reference.py`). The ARC knobs are:

```bash
# ARC store selection
CLIO_ARC_STORE=cte                 # "cte" (clio-core, default) or "local" (file-based)
CLIO_ARC_STORE_CONFIG=             # path to a clio-core CTE config; blank = auto

# In-memory cache + LSM index
CLIO_ARC_CACHE_CAPACITY=1000       # LRU cache capacity (entries)
CLIO_ARC_LSM_MEMTABLE_SIZE=1000    # LSM memtable size before flush
CLIO_ARC_LSM_COMPACTION_THRESHOLD=5  # segments before a compaction

# CTE spillover
CLIO_ARC_CTE_DIR=                  # CTE working dir; blank = OS data dir
CLIO_ARC_CTE_FILE_CAPACITY=50GB    # on-disk capacity for the CTE hierarchy
```
