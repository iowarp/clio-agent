...
├── registry/                 # Agent Registry (v0.2.0)
│   ├── registry.py           # Capability-based routing
│   └── a2a_adapter.py        # A2A protocol adapters
├── arc/                      # Memory layer (v0.2.0+)
...
│   ├── mcp_wrapper.py        # MCP client
│   └── servers/              # MCP server implementations
│       └── hdf5_server.py    # HDF5 operations (v0.3.0+)
└── ui/
...
**Total**: ~5,000+ lines of intelligent, self-improving agent code
...
/clear     - Clear history
/tune      - Enter offline tuning mode (v0.4.0+)
/quit      - Exit
```
...
```python
# OpenAI
lm = setup_lm(provider="openai", model="gpt-4")

# Anthropic
lm = setup_lm(provider="anthropic", model="claude-3-5-sonnet-20241022")

# Google
lm = setup_lm(provider="google", model="gemini-pro")
```
...
- Main agent orchestration (Tier 1) with conversation management
- DataExpert agent (Tier 2) with ReAct pattern
- LM configuration for any provider (LM Studio, Ollama, OpenAI, etc.)
- UV self-contained script architecture
- Interactive CLI with Rich TUI
- Dual-LM support (separate models for routing vs. reasoning)

### 🚧 In Progress (v0.2.0 - Intelligence + Memory)

- **Agent Registry implementation**: Capability-based coordination
- **A2A protocol specification & adapters**: External agent integration
- **External agent compilation**: LangChain, CrewAI, AutoGen support
- **ARC Memory Layer foundation**: In-memory cache + B-tree indexing
- **Conversation storage**: Persistent context management

### 📋 Planned

**Phase 3 (v0.3.0) - Tools & IOWarp Integration**:
- FastMCP server implementations (HDF5, SLURM, Darshan, etc.)
- CAE/PPI layer integration with IOWarp
- **ARC-CTE integration**: Persistent storage in IOWarp namespace
- CTE layer coordination for multi-tier storage
- Tool result caching in ARC

**Phase 4 (v0.4.0) - Advanced Patterns + Learning**:
- Nanoagent spawning (Tier 3)
- Hierarchical expert delegation
- Parallel expert execution
- **Optimizer Layer implementation**: Offline tuning mode
- **Performance tracking**: Metrics collection in ARC
- **Prompt optimizers**: BootstrapFewShot, MIPRO
- AgentLog audit trails (Spring 2025)

**Phase 5 (v0.5.0) - Pre-Production Public Beta**:
- Improved CLI with tuning mode
- REST API (FastAPI server)
- **Online learning mode**: A/B testing, automatic optimization
...
- Integrated Agent Toolkit (15+ servers, 150+ tools)
...
- **[DSPy](https://github.com/stanfordnlp/dspy)** - DSPy: Programming—not prompting—Foundation Models
