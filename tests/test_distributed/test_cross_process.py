...
async def test_happy_path_cross_process(cross_arc, spawn_worker):
    ...
    mb = ClioCoreMailbox(cross_arc, prefix=prefix)
    res = await ClioCoreExpertInvoker(mb, timeout=30).invoke(ExpertRequest("data", "ping"))
...
async def test_worker_crash_single_worker_parent_times_out(cross_arc, spawn_worker):
    ...
    with pytest.raises(TimeoutError):
        await ClioCoreExpertInvoker(mb, timeout=6).invoke(ExpertRequest("data", "will-orphan"))
...
async def test_worker_crash_other_worker_reclaims(cross_arc, spawn_worker):
    ...
    spawn_worker(prefix, mode="echo", n=1)
    mb = ClioCoreMailbox(cross_arc, prefix=prefix)
    res = await ClioCoreExpertInvoker(mb, timeout=30).invoke(ExpertRequest("data", "reclaim-me"))
...
async def test_concurrent_delegations_across_worker_processes(cross_arc, spawn_worker):
    ...
    mb = ClioCoreMailbox(cross_arc, prefix=prefix)
    inv = ClioCoreExpertInvoker(mb, timeout=40)
    n = 12
...
async def test_scale_many_delegations_across_workers(cross_arc, spawn_worker):
    ...
    mb = ClioCoreMailbox(cross_arc, prefix=prefix)
    inv = ClioCoreExpertInvoker(mb, timeout=60)
    n = 30
...
async def test_large_payload_crosses_cte(cross_arc, spawn_worker):
    ...
    mb = ClioCoreMailbox(cross_arc, prefix=prefix)
    big = "X" * 200_000
    res = await ClioCoreExpertInvoker(mb, timeout=30).invoke(ExpertRequest("data", big))
...
async def test_async_fanout_split_pools_cross_process(cross_arc, spawn_worker):
    ...
    stage_inv = ClioCoreExpertInvoker(ClioCoreMailbox(cross_arc, prefix="dp_stage_"), timeout=60)
    anal_inv = ClioCoreExpertInvoker(ClioCoreMailbox(cross_arc, prefix="dp_anal_"), timeout=60)
...
async def test_lease_prevents_double_execution(cross_arc, spawn_worker):
    ...
    mb = ClioCoreMailbox(cross_arc, prefix=prefix)
    res = await ClioCoreExpertInvoker(mb, timeout=30).invoke(ExpertRequest("data", "once"))
...
async def test_real_alcf_expert_cross_process(cross_arc, spawn_worker):
    ...
    mb = ClioCoreMailbox(cross_arc, prefix=prefix)
    res = await ClioCoreExpertInvoker(mb, timeout=150).invoke(
...
async def test_heterogeneous_alcf_models_cross_process(cross_arc, spawn_worker):
    ...
    s = await ClioCoreExpertInvoker(ClioCoreMailbox(cross_arc, prefix="xp_s_"), timeout=150).invoke(
    ...
    try:
        m = await ClioCoreExpertInvoker(ClioCoreMailbox(cross_arc, prefix="xp_m_"), timeout=150).invoke(
...
async def test_ndp_pipeline_multihop_cross_process(cross_arc, spawn_worker):
    ...
    geo = await ClioCoreExpertInvoker(ClioCoreMailbox(cross_arc, prefix="xp_geo_"), timeout=150).invoke(
    ...
    dd = await ClioCoreExpertInvoker(ClioCoreMailbox(cross_arc, prefix="xp_dd_"), timeout=150).invoke(
        ExpertRequest(
            "data",
...
async def test_clio_core_worker_subprocess_over_daemon(cross_arc, clio_daemon, tmp_path):
    ...
    actual transport (the LocalFS twin lives in test_clio_core_worker)."""
    prefix = "clio_core_wkr_"
    ...
    try:
        invoker = ClioCoreExpertInvoker(ClioCoreMailbox(cross_arc, prefix=prefix), timeout=200, poll=0.2)
