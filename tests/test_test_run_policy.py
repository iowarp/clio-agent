...
def test_cleanup_rejects_paths_outside_owned_base(tmp_path: Path) -> None:
    ...
    outside.mkdir()
    (outside / "owner.json").write_text(json.dumps({"schema": SCHEMA, "checkout": str(CHECKOUT)}))
...
def test_pytest_state_is_inside_the_lease(tmp_path: Path, pytestconfig: pytest.Config) -> None:
...
def test_runner_cleanup_on_success_and_failure(tmp_path: Path, code: int) -> None:
    ...
    spec = spec_from_file_location("owned_test_runner", CHECKOUT / "scripts" / "run_tests.py")
