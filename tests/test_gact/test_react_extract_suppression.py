...
def test_return_contract_carries_answer_while_visible_parts_suppressed(tmp_path, monkeypatch) -> None:
    ...
    Prediction — assert ``result.answer`` equals the extract answer BYTE-FOR-BYTE
    (this Prediction is what delegation reads into ``row["output"]``, the *show
    more* source), while NO visible reasoning/answer part reached the transcript."""
    ...
    def fake_trunc(module, trajectory, **input_args):
        ...
        # The EXTRACT: in production its tokens stream through the tap. Reproduce
        # that here — both fields must be suppressed on the wire (react kind) — then
        # return the real extract fields the Prediction carries.
        note_lm_answer_delta(REASONING, field="reasoning")
        note_lm_answer_delta(ANSWER, field="answer")
        return _AttrDict(answer=ANSWER, reasoning=REASONING)

    inst._call_with_potential_trajectory_truncation = fake_trunc

    def action():
        _ctx.install_trajectory_cell()
        return inst.forward(question="find stations near San Diego")
    ...
    # VALUE preserved byte-for-byte on the return contract.
    assert result.answer == ANSWER
    # PARTS suppressed on the wire (and on reload).
    assert _visible_parts(facts["transcript"], "reasoning") == []
    assert _visible_parts(facts["transcript"], "answer") == []
    persisted = _persisted_text_fields(facts["client"], facts["sid"])
...
class _AttrDict(dict):
    """dict whose keys are attribute-accessible (mimics dspy.Prediction)."""
