...
def test_role_queue_is_the_default_and_ignores_hints():
    ...
    assert p.mailbox_for("analysis") == "cee_analysis"
    assert p.mailbox_for("data", hints={"node": "n2"}) == "cee_data"  # caller decoupled from node


def test_node_affinity_routes_to_a_node_queue_when_hinted():
    p = make_placement("affinity")
    assert p.mailbox_for("data") == "cee_data"  # no hint -> plain role queue
    assert p.mailbox_for("data", hints={"node": "n2"}) == "cee_n2_data"  # "run where the data is"
