"""Unit tests for the durable ChatGPT credential store (A.4, A.9)."""
...
from clio_agent.providers.chatgpt.oauth import OAuthError, TokenResponse


def _credential(*, expires_in_ms: int = 60 * 60 * 1000) -> ChatGptCredential:
...
def test_save_persists_at_0600(tmp_path: Path) -> None:
    path = tmp_path / "chatgpt_credential.json"
    store = ChatGptCredentialStore(path=path)
    store.save(_credential())
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["schema"] == "clio-agent.chatgpt-credential.v1"
...
def test_get_valid_credential_raises_when_missing(tmp_path: Path) -> None:
    store = ChatGptCredentialStore(path=tmp_path / "chatgpt_credential.json")
    with pytest.raises(ChatGPTCredentialMissingError):
...
def test_get_valid_credential_returns_fresh_without_refresh(
    ...
    monkeypatch.setattr(
        "clio_agent.providers.chatgpt.credentials._refresh_token_request", _fail_refresh
    )
...
def test_get_valid_credential_refreshes_when_near_expiry(
    ...
    monkeypatch.setattr("clio_agent.providers.chatgpt.credentials._refresh_token_request", _refresh)
    monkeypatch.setattr(
        "clio_agent.providers.chatgpt.credentials.decode_account_id", lambda _token: "acct_1"
    )
    result = store.get_valid_credential()
    assert result.access_token == "new_at"
    assert result.refresh_token == "new_rt"
    # Refresh rotation is persisted atomically (A.4): a fresh store reload sees it.
    reloaded = ChatGptCredentialStore(path=tmp_path / "chatgpt_credential.json").load()
...
def test_refresh_failure_raises_typed_error_and_does_not_corrupt_store(
    ...
    monkeypatch.setattr("clio_agent.providers.chatgpt.credentials._refresh_token_request", _fail)
    with pytest.raises(ChatGPTRefreshFailedError):
        store.get_valid_credential()
