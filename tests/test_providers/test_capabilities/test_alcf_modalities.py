...
async def test_embedding_and_segmentation_models_are_listed_by_type_not_for_chat() -> None:
    ...
    assert sfr["model_type"] == "embedding"  # HF pipeline_tag=feature-extraction
    assert sfr["chat_selectable"] is False
    assert sfr["availability"] == NOT_CHAT_AVAILABILITY
    sfr_type = get_effective_capabilities(PROVIDER_ID, SOPHIA, "Salesforce/SFR-Embedding-Mistral")
    assert sfr_type.model_type.source == "hf_repo"
