...
disconnect), B2 (warm-pool claim reconciliation), B17 (dead client replaced
from the warm pool).
...
# --------------------------------------------------------------------------- #
async def test_warm_claim_with_matching_config_is_free(monkeypatch: pytest.MonkeyPatch) -> None:
    """A session that wants exactly the warm defaults (no model/cwd/thinking/
    system_prompt override) pays nothing extra when claiming a warm entry."""
    state = _install_fake_sdk(monkeypatch)
    ...
    await warm_entry._ensure_client(lambda: None)  # pre-connect with bare defaults
    pool._warm.append(warm_entry)
    assert state["constructed"] == 1

    entry = pool.entry_for(session_id="sess-bare")
    assert entry is warm_entry
    await _consume(
        entry,
        payload="p",
        native_blocks=[],
    ...
    assert state["constructed"] == 1  # no reconnect -- the claim was free


async def test_warm_claim_with_a_model_only_difference_uses_set_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    state = _install_fake_sdk(monkeypatch)
    pool = ClaudeStreamClientPool(max_concurrent=4, warm_size=0)
    monkeypatch.setattr(pool, "_spawn_warm_refill", lambda: None)
    warm_entry = _StreamClientEntry(connect_slots=pool._connect_slots)
    await warm_entry._ensure_client(lambda: None)  # bare defaults, model=None
    pool._warm.append(warm_entry)

    entry = pool.entry_for(session_id="sess-model")
    await _consume(
        entry,
        payload="p",
        native_blocks=[],
    ...
    assert state["constructed"] == 1  # still no reconnect
...
# --------------------------------------------------------------------------- #
# B17: a dead entry's session is handed a warm replacement, typed and logged.
# --------------------------------------------------------------------------- #
def test_dead_entry_is_replaced_from_the_warm_pool(
    ...
    assert replacement is warm_entry
    assert replacement is not dead_entry
    assert pool._warm == []
    assert any("dead_client_replaced" in rec.getMessage() for rec in caplog.records)


def test_dead_entry_replacement_mints_cold_when_warm_pool_is_empty(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    pool = ClaudeStreamClientPool(max_concurrent=4, warm_size=0)
    monkeypatch.setattr(pool, "_spawn_warm_refill", lambda: None)

    dead_entry = pool.entry_for(session_id="sess-y")
    dead_entry._dead = True

    replacement = pool.entry_for(session_id="sess-y")
    assert replacement is not dead_entry
