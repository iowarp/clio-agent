...
def test_benchmark_has_exactly_twelve_case_contracts() -> None:
    case_dirs = sorted(path for path in BENCHMARK.iterdir() if path.is_dir())
    ...
    for case_dir in case_dirs:
        ...
        assert "Status: not passed." in text
...
def test_case_matrix_rejects_old_shortcut_semantics() -> None:
    matrix = (BENCHMARK / "CASE_MATRIX.md").read_text(encoding="utf-8")
    earthscope_case = (
